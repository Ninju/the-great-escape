...
(define (set-sprite-heading s x y)
  (make-sprite (sprite-x s)
               (sprite-y s)
               (set-particle-heading (sprite-particle s) x y)
               (sprite-image s)))

(define (make-normalized-vector2 x y)
  (normalize-vector2 (make-vector2 x y)))

(define (set-particle-heading p x y)
  (make-particle (particle-speed p)
                 (make-normalized-vector2 (- x (vector2-x (particle-direction p)))
                                          (- y (vector2-y (particle-direction p))))))
...
(define (update-sprite s)
  (let* ((particle (sprite-particle s))
         (speed (particle-speed particle))
         (dxy (particle-direction particle))
         (dx (vector2-x dxy))
         (dy (vector2-y dxy)))
  (make-sprite (+ (* dx speed) (sprite-x s))
               (+ (* dy speed) (sprite-y s))
               (sprite-particle s)
               (sprite-image s))))
