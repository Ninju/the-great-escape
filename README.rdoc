...
It's a game (or at least, will be!) written in the Scheme programming language. You are a prisoner on the run. The goal is to escape by completing in the levels in which you run past various officers (prison officers, police officers, border patrol officers (is that the right name?) etc etc).
...
Essentially what I'm saying is that I don't really know the best way to describe this, but I am using PLT Scheme. Development done mostly in DrScheme using the "Advanced Student" language (/teachpack??? see what I mean)... so about the only advice I can give for running it, is to open DrScheme and open the file from there, and that should work - provided you are using a relatively new version (v372 didn't work for me, and I am using v 4.2.4 right now - which does work).
